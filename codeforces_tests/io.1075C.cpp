...
istream& operator>>(istream &os, tuple<ll, ll, ll>& thing) {
	return os >> get<0>(thing) >> get<1>(thing) >> get<2>(thing);
}

MAIN main() {
	cin >> n >> m;
	ver = mytl::readValues<ll>(n);
	hor = mytl::readValues<tuple<ll, ll, ll>>(m);
	map<ll, ll> hol_meddig;
	sort(begin(hor), end(hor));

	for (auto [l, r, height] : hor) {
		if (hol_meddig[height] + 1 == l) hol_meddig[height] = r;
	}

	vector<ll> takarasok;
	ll adka = 0;
	for (auto elem : hol_meddig) {
		if (elem.second == 1000000000LL) {
			adka++;
			continue;
		}
		if (elem.second != 0) takarasok.push_back(elem.second);
	}

	sort(takarasok.begin(), takarasok.end());
	sort(ver.begin(), ver.end());

	ll ans = ver.size();

	ll latest_that_can_be_freed = -1;
	// cout<<takarasok<<" "<<ver<<endl;
	for (ll farthest_vertival_to_delete = -1; farthest_vertival_to_delete + 1 < ver.size(); farthest_vertival_to_delete++) {

		while (latest_that_can_be_freed + 1 < takarasok.size() && takarasok[latest_that_can_be_freed + 1] < ver[farthest_vertival_to_delete + 1]) latest_that_can_be_freed++;
		//WATCH(latest_that_can_be_freed);

		ans = min(ans, (ll)(takarasok.size() - latest_that_can_be_freed - 1 + farthest_vertival_to_delete + 1));
	}
