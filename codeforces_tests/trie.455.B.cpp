...
using T = mytl::TrieNode<char, unordered_map<char, void*> >;

T root;

map<T*, bool> win_d, loose_d;

bool can_win(T* const curr) {
	if (win_d.find(curr) == win_d.end()) {
		bool res = false;
		for (auto const& p : curr->next) {
			if (!can_win((T*)(p.second))) {
				res = true;
				break;
			}
		}
...
bool can_loose(T* const curr) {
	if (loose_d.find(curr) == loose_d.end()) {
		bool res = false;
		if (curr->next.empty()) {
			res = true;
		}
...
MAIN main(){
	...
	for (ll i = 1; i <= n; i++) {
		string be;
		cin >> be;
		root.traverse(vector<char>(be.begin(), be.end()));
	}


	bool W = can_win(&root), L = can_loose(&root);
