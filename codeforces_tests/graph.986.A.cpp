...
using G = mytl::NormalSimpleGraph;
...
vector<vector<ll> > tav(101, vector<ll>(100001));
...
MAIN main(){
    ...
    for(ll i=1; i<=n; i++){
        ll csop;
        cin>>csop;
        csopi[csop].push_back(i);
    }

    G g;
    mytl::readEdgeList(g, m);
    
    for(ll CS=1; CS<=k; CS++){
        vector<pair<ll, ll> > x;
        for(ll elem : csopi[CS]){
            x.push_back({0, elem});
        }

        tav[CS] = mytl::queue_graph_algorithm<G, mytl::FIFO, mytl::SimpleJustLength>(g, x);
        //cout<<CS<<": "<<tav[CS]<<endl;
    }

    for(ll i=1; i<=n; i++){
        vector<ll> akt;
        for(ll j=1; j<=k; j++) akt.push_back(tav[j][i]);
        sort(akt.begin(), akt.end());
        long long aktv = 0;
        for(ll j=0; j<s; j++) aktv += akt[j];
        cout<<aktv<<" ";
    }
