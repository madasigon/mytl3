...
MAIN main(){
    cin>>n>>m;
    auto coords = mytl::readValues<ll>(n+m);
    auto is_taxi = mytl::readValues<ll>(n+m);
    vector<ll> taxi_drivers, people;
    for(ll i : mytl::forrange((ll)n+m, 0LL)){
        if(is_taxi[i]) taxi_drivers.push_back(coords[i]);
        else people.push_back(coords[i]);
    }

    ll current_closest = 0;
    vector<ll> points(m);
    for(ll person : people){
        while(current_closest+1 < taxi_drivers.size() && abs(person - taxi_drivers[current_closest]) > abs(person - taxi_drivers[current_closest+1])) current_closest++;
        points[current_closest]++;
    }
