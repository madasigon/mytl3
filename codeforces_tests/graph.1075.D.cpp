...
struct LengthAndLastNode{
    ...
    static Info append(pair<Info, typename G::Node> from, typename G::Edge e, typename G::Node n){
        return Info(from.first + e, from.second);
    }
};
...
struct SimpleLengthAndLastNode{
    using Info = pair<ll, typename G::Node>;
    static Info append(pair<Info, typename G::Node> from, typename G::Edge e, typename G::Node n){
        return Info(from.first.first + 1, from.second);
    }
};
...
using G = mytl::Container_Graph<ll, Void, mytl::AssocVector>;
...
void processs(){
    ...
    cin>>n;
    G g(n);
    mytl::readEdgeList(g, n-1);

        //cout<<"hh";
    auto enyem_ = mytl::readValues<ll>(mytl::read<ll>());
    set<ll> enyem(enyem_.begin(), enyem_.end());
    auto ove_ = mytl::readValues<ll>(mytl::read<ll>());
    set<ll> ove(ove_.begin(), ove_.end());

    //cout<<enyem<<" "<<ove<<endl;
    auto ask = [&](ll& ide, char tipus, ll ki) -> bool{
        cout<<tipus<<" "<<ki<<endl;
        ll got;
        cin>>got;
        if(tipus == 'A' && mytl::has_key(ove, got)){
            cout<<"C "<<ki<<endl;
            return true;
        }
        if(tipus == 'B' && mytl::has_key(enyem, got)){
            cout<<"C "<<got<<endl;
            return true;
        }
        ide = got;
    ...
    ll one_piece;
    if(ask(one_piece, 'B', *ove.begin())) return;
     vector<pair<pair<ll,ll>, ll> > sour{{{0,-1}, *enyem.begin()}};
    // cout<<sour<<endl;
   auto tb = mytl::queue_graph_algorithm<G, mytl::AlgoComposer<mytl::FILO,mytl::SimpleLengthAndLastNode>::A >(g,sour);



    while(!mytl::has_key(enyem, one_piece)){
        one_piece = tb[one_piece].second;
    }


    ll dc;
    if(ask(dc, 'A', one_piece)) return;
    else{
        cout<<"C -1"<<endl;
    }
