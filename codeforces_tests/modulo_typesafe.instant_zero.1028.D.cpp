...
MAIN main(){
    ...
    ll n;
    cin>>n;
    map<ll, Resetter<Mod107 >::Variable > nodok_;

    nodok_[0] = 1;
    nodok_[1000000000LL] = 0;
    ...
    for(ll i=1; i<=n; i++){
        ...
        if(szo == "ADD"){
            nodok_[val] = 0;
            nodok_[val] = prev(nodok_.find(val))->second.get();

            //cout<<get_from(nodok[val])<<" -> "<<nodok_[val].get()<<endl;
        }
        else{
            auto talal_ = prev(nodok_.find(val));
            talal_->second = talal_->second.get() + nodok_[val].get();
            global_resetter<Mod107>->reset(0);
            talal_->second.survive();
            nodok_.erase(val);

        }
        //cout<<"most "<<latest<<endl;
        //for(auto &par : nodok){
        //    cout<<par.first<<": "<<par.second.d<<" "<<par.second.friss<<endl;
        //}
    }
    {
    Mod107 res = 0;
