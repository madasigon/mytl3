...
struct Custom_Op {
	...
	static T apply(ll dontcare, T a, Change c) {
		if (c != -2) return c;
		else return a;
	}
...
struct Case {
	...
	void main() {
		...
		d.resize(n + 1, 1);
		mytl::SegmentTree<Custom_Op> tree({ 1, n }, [&](ll i) {
			return i;
		});
		...
		for (ll i = 1; i <= n; i++) maxd.insert({ 1, i });

		for (ll i__ = 1; i__ <= n; i__++) {
			ll pozkell = hova[i__];
			if (d[pozkell] < prev(maxd.end())->first) {
				cout << "No" << endl;
				return;
			}
			ll akt_hova = pozkell < n ? tree.query_single(pozkell+1) : n + 1;
			tree.modify({ pozkell + 1 - d[pozkell], pozkell }, akt_hova);

			if (akt_hova <= n) {
				maxd.erase({ d[akt_hova], akt_hova });
				d[akt_hova] = akt_hova - (pozkell + 1 - d[pozkell]) + 1;
				maxd.insert({ d[akt_hova], akt_hova });
			}

			maxd.erase({ d[pozkell], pozkell }); // nem kell visszarakni, mert úgyis -1
			d[pozkell] = 0;
		}
